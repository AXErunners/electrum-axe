...
# try to make console log lines short... no timestamp, short levelname, no "electrum_axe."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_axe."):
...
# creates a logger specifically for electrum_axe library
electrum_logger = logging.getLogger("electrum_axe")
electrum_logger.setLevel(logging.DEBUG)


def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_axe_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_axe_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_axe."):
        name = name[14:]
