...
The Kivy GUI is used with AXE Electrum on Android devices. To generate an APK file, follow these instructions.

## 1. Preliminaries

Make sure the current user can write `/opt` (e.g. `sudo chown username: /opt`).

We assume that you already got AXE Electrum to run from source on this machine,
...
p4a is used to package AXE Electrum, Python, SDL and a bootstrap Java app into an APK file. 
We patched p4a to add some functionality we need for AXE Electrum. Until those changes are
...
In the `gui/kivy` directory of AXE Electrum, run `make theming`.

## 6. Download AXE Electrum dependencies
```sh
