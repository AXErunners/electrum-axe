...
from electrum_axe.axe_net import MIN_PEERS_LIMIT, MAX_PEERS_LIMIT
...
class AxePeersWidget(QTreeWidget):
    ...
    def create_menu(self, position):
        ...
        if not axe_net.use_static_peers:
    ...
    def update(self, event=None, args=None):
        ...
        if event is None:
            ...
            for peer, axe_peer in sorted(list(peers.items())):
                self.add_peer(peer, axe_peer)
        elif event == 'axe-peers-updated':
            action, peer = args
            if action == 'added':
                axe_peer = peers.get(peer)
                if axe_peer:
                    self.add_peer(peer, axe_peer, insert=True)
            elif action == 'removed':
                items = self.findItems(peer, MATCH_STR_CS)
                if items:
                    idx = self.indexOfTopLevelItem(items[0])
                    self.takeTopLevelItem(idx)
        elif event == 'axe-net-activity':
            for peer, axe_peer in sorted(list(peers.items())):
                items = self.findItems(peer, MATCH_STR_CS)
                if items:
    ...
    def add_peer(self, peer, axe_peer, insert=False):
        axe_net = self.parent.network.axe_net
        peers = axe_net.peers
        v = axe_peer.version
...
class SporksWidget(QTreeWidget):
    ...
    def update(self):
        axe_net = self.parent.network.axe_net
        sporks_dict = axe_net.sporks.as_dict()
        self.clear()
...
class AxeNetDialogLayout(object):
    ...
    def __init__(self, network, config, parent):
        ...
        tabs.addTab(axe_net_tab, _('Axe Network'))
        ...
        if parent.is_testnet:
            ...
            def update_bls_speed():
                if self.parent.isVisible() and bls_speed_tab.isVisible():
                    start_t = time.time()
                    res = self.network.axe_net.test_bls_speed()
        ...
        self.run_axe_net_cb = QCheckBox(_('Enable Axe Network'))
        ...
        # row 1
        is_cmd_axe_peers = axe_net.is_cmd_axe_peers
        use_static_peers = axe_net.use_static_peers
        ...
        def on_axe_peers_editing_end():
            if is_cmd_axe_peers:
                return
            res = axe_net.axe_peers_from_str(self.axe_peers_e.text())
            if type(res) == str:
        ...
        # row 4
        self.axe_peers_list = AxePeersWidget(self)
        grid.addWidget(self.axe_peers_list, 4, 0, 1, -1)

        # Axe Sporks tab
        vbox = QVBoxLayout(sporks_tab)
        sporks_label = QLabel(_('Axe Sporks Values'))
        ...
        banlist_label = QLabel(_('Banned Axe Peers'))
    ...
    def update(self, event=None, args=None):
        ...
        if event is None:
            self.update_axe_net_tab()
            self.sporks_list.update()
            self.banlist_list.update()
        elif event in ['axe-peers-updated', 'axe-net-activity']:
            self.update_axe_net_tab(event, args)
        elif event == 'sporks-activity':
            self.sporks_list.update()
        elif event == 'axe-banlist-updated':
            self.banlist_list.update(event, args)

    def update_axe_net_tab(self, event=None, args=None):
        axe_net = self.network.axe_net
        self.axe_peers_list.update(event, args)
        if event in [None, 'axe-net-activity']:
            read_bytes = axe_net.read_bytes
            write_bytes = axe_net.write_bytes
        ...
        if event in [None, 'axe-peers-updated']:
            status = _('Connected Peers') + f': {len(axe_net.peers)}'
...
class AxeNetDialog(QDialog):
    def __init__(self, network, config, axe_net_sobj):
